...
export default function PageClient() {
  useEffect(() => {
    // Remove no-js class (client-side hydration)
    document.documentElement.classList.remove("no-js");

    // Footer year injection (replaces inline document.write)
    ...
    if (!isTouchDevice) {
      document.querySelectorAll<HTMLElement>(".ben").forEach((card) => {
        const move = (e: MouseEvent) => {
    ...
    w.handleSubmit = (e: Event) => {
      ...
      fetch("https://formspree.io/f/mnjbbynk", {
        method: "POST",
    ...
    // ——— CAL.COM EMBED ———
    // Inject the stock Cal.com embed loader. Running the minified original
    // via an injected <script> avoids re-typing a dynamic self-referencing
    // IIFE inside strict TypeScript.
